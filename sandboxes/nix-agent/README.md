...
Nix-ready Docker Sandbox templates. Extends official `docker/sandbox-templates:<variant>` so agent wiring and the sbx credential proxy stay intact; Nix is layered on top for flaked projects.
...
Stack mixin network allowlists from [`../kits/`](../kits/) with `--kit`:
...
Copy `sandboxes/kits/espressif` into a project as `sbx/espressif` when needed. Prefer kits over one-off `sbx policy allow network "..."`.
